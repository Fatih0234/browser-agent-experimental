...
## Portal Gym Project - Local Notes

### Project Identity
- This repository builds a **portal-gym benchmark platform**
- Phase 1 is a **shared shell only** — no portal-specific workflows yet
- This is for browser automation validation, not a production business portal

### Architecture Principles
- Schema first, UI second
- Reuse shared primitives
- No extra infra beyond Vercel + Supabase
- No Redis, no message queues, no microservices, no separate workers
- No browser-use runtime yet
- Build in small slices with verification after each slice
- Keep code simple and Vercel-friendly
- Prefer server actions / simple API routes over overengineering
- All data access must respect organization boundaries

### Data Boundaries
- Every query must filter by `org_id` where applicable
- Use Row Level Security (RLS) policies in Supabase
- Admin-only operations require explicit role checks
...
│   ├── (auth)/            # Auth group (sign-in, sign-out)
...
├── playwright-tests/     # Playwright smoke tests
└── docs/                 # Architecture notes
```

### Development Workflow
- Build in small milestones
- Make plans explicit before coding
